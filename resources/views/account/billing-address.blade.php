        ...
        <div>
            ...
            <div class="mb-6">
                ...
                <select id="country" name="country" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
                    ...
                    {{ isset($billingAddress) && $billingAddress->country_id == $country->id ? 'selected' : '' }}>
                    {{ $country->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="text" id="address_line_1" name="address_line_1" value="{{ isset($billingAddress) ? $billingAddress->address_line_1 : '' }}" placeholder="House number and street name" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
            </div>

            <div class="mb-6">
                <label for="address_line_2" class="block text-sm font-montserrat font-medium text-gray-700 mb-2">Address Line 2</label>
                <input type="text" id="address_line_2" name="address_line_2" value="{{ isset($billingAddress) ? $billingAddress->address_line_2 : '' }}" placeholder="Apartment, suite, unit etc." class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
            </div>
            ...
            <div class="grid grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <select id="city" name="city" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
                        ...
                        <option value="{{ $city->id }}" {{ isset($billingAddress) && $billingAddress->city_id == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select id="state" name="state" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
                        ...
                        <option value="{{ $state->id }}" {{ isset($billingAddress) && $billingAddress->state_id == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <input type="text" id="zip_code" name="zip_code" value="{{ isset($billingAddress) ? $billingAddress->zip_code : '' }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
                </div>
            </div>
