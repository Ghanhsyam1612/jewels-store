<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto space-y-8">
        <!-- Status Banner -->
        <div class="{{ $subscription && $subscription->isActive() ? 'bg-green-100 border-green-500' : 'bg-red-100 border-red-500' }} border-l-4 p-4 mb-4">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <svg class="h-5 w-5 {{ $subscription && $subscription->isActive() ? 'text-green-500' : 'text-red-500' }}" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    </svg>
                </div>
                <div class="ml-3">
                    <p class="text-sm {{ $subscription && $subscription->isActive() ? 'text-green-700' : 'text-red-700' }}">
                        @if($subscription && $subscription->isActive())
                            Your subscription is currently active and will renew on {{ $subscription->current_period_end->format('M d, Y') }}
                        @elseif($subscription && $subscription->isOnGracePeriod())
                            Your subscription is on grace period until {{ $subscription->current_period_end->format('M d, Y') }}
                        @else
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    <div class="space-y-4">
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="text-2xl font-bold text-indigo-600">${{ number_format($subscription->amount ?? 29.99, 2) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 rounded-xl p-6">
                        ...
                        <ul class="space-y-3">
                            <li class="flex items-center">
                                ...
                                <span class="ml-3 text-gray-700">Premium Features Access</span>
                            </li>
                            <li class="flex items-center">
                                <span class="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full bg-indigo-100 text-indigo-600">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
