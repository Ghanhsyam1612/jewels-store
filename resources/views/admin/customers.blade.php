                        ...
                        <div x-show="isModalOpen" class="fixed inset-0 flex items-center justify-center p-5 overflow-y-auto modal z-99999">
                            ...
                            <div class="relative w-full max-w-[584px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10" @click.outside="isModalOpen = false">
                                ...
                                <form :action="isEditMode ? '/admin/customers/' + customerId : '/admin/customers'" method="POST">
                                    ...
                                    <button type="submit" class="mt-6 w-full rounded-lg bg-black px-4 py-2 text-white hover:bg-blue-700" x-text="isEditMode ? 'Update Customer' : 'Save Customer'"></button>
                                </form>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex flex-row items-center gap-2">
                                            <form action="{{ route('admin.customers.destroy', $customer->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this customer?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="!text-red-500  dark:text-gray-400 dark:hover:text-error-500 mt-1.5">
                                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                    </svg>
                                                </button>
                                            </form>
                                            <button @click="isModalOpen = true; isEditMode = true; customerId = {{ $customer->id }}; loadCustomer({{ $customer->id }})" class="!text-gray-800 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
                                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
