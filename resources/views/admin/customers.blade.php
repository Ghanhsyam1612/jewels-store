                    ...
                    <div x-data="{ isModalOpen: false, isEditMode: false, customerId: null }">
                        ...
                        <div x-show="isModalOpen" class="fixed inset-0 flex items-center justify-center p-5 overflow-y-auto modal z-99999">
                            ...
                            <div class="relative w-full max-w-[584px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10" @click.outside="isModalOpen = false">
                                ...
                                <form x-bind:action="isEditMode ? '{{ route('admin.customers.index') }}/' + customerId : '{{ route('admin.customers.store') }}'" method="POST">
                                    @csrf
                                    ...
                                    <button type="submit" class="mt-6 w-full rounded-lg bg-blue-600 px-4 py-2 text-black hover:bg-blue-700" x-text="isEditMode ? 'Update Customer' : 'Save Customer'"></button>
                                </form>
                            </div>
                        </div>
                        <!-- End Model -->
                        ...
                        <thead>
                            <tr class="border-y border-gray-200 bg-gray-50 text-left dark:border-gray-800 dark:bg-gray-900">
                                @foreach(['first_name' => 'First Name', 'last_name' => 'Last Name', 'display_name' => 'Display Name', 'email' => 'Email', 'phone' => 'Phone', 'is_email_verified' => 'Email Verified', 'created_at' => 'Created At'] as $key => $label)
                                    <th class="px-4 py-3">
                                        <a href="{{ route('admin.customers.index', array_merge(request()->query(), ['sort' => $key, 'direction' => $sortColumn == $key && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}" class="flex cursor-pointer items-center gap-2">
                                            <span class="text-sm font-medium text-gray-700 dark:text-gray-400">{{ $label }}</span>
                                            <span class="flex flex-col gap-0.5">
                                                <svg class="h-2 w-2 {{ $sortColumn == $key && $sortDirection == 'asc' ? 'fill-blue-500' : 'fill-gray-400' }}" viewBox="0 0 8 5">
                                                    <path d="M4 0l4 5H0z" />
                                                </svg>
                                                <svg class="h-2 w-2 {{ $sortColumn == $key && $sortDirection == 'desc' ? 'fill-blue-500' : 'fill-gray-400' }}" viewBox="0 0 8 5">
                                                    <path d="M4 5l4-5H0z" />
                                                </svg>
                                            </span>
                                        </a>
...
<script>
    function loadCustomer(id) {
        fetch(`{{ route('admin.customers.index') }}/${id}/edit`)
            ...
            .then(data => {
                ...
                // Set select option
                const selectElement = document.getElementById('is_email_verified');
                selectElement.value = data.is_email_verified ? '1' : '0';
            })
            .catch(error => {
