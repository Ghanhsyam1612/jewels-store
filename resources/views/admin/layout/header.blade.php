                ...
                <form action="https://formbold.com/s/unique_form_id" method="POST">
                    <div class="relative flex items-center">
                        ...
                        <button class="absolute left-4 flex items-center">
                            <svg class="fill-gray-500 dark:fill-gray-400 w-5 h-5" viewBox="0 0 20 20" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
        ...
        <div :class="menuToggle ? 'flex' : 'hidden'"
            ...
            <div class="flex items-center gap-2 2xsm:gap-3">
                ...
                <div class="relative" x-data="{
                    ...
                    displayCount: 0,
                    animateCount() {
                        let start = this.displayCount;
                        let end = this.unreadCount;
                        let current = start;
                ...
                setInterval(() => fetchNotifications(), 60000)"
                    ...
                    <button
                        ...
                        <span :class="!notifying ? 'hidden' : 'inline-flex'"
                            class="absolute -right-1 -top-1 min-w-[20px] h-5 items-center justify-center rounded-full bg-black text-xs font-medium text-white px-1">
                            <span x-text="displayCount" class="mx-auto"></span>
                        </span>
                    ...
                    <div x-show="dropdownOpen"
                        class="absolute right-0 mt-2 w-80 flex flex-col rounded-lg border border-gray-200 bg-white p-4 shadow-lg">
                        <div class="mb-4">
                            <h5 class="text-lg font-semibold text-gray-800">
                                Notification (<span x-text="unreadCount"></span>)
                            </h5>
                        </div>
                        ...
                        <ul class="flex flex-col">
                            <template x-for="notification in notifications" :key="notification.id">
                                <li>
                                    <a class="flex gap-3 p-2 hover:bg-gray-100 rounded" href="#"
                                        @click.prevent="markAsRead(notification.id)">
                                        <span class="block">
                                            <span class="block text-sm text-gray-600" x-text="notification.message">
                                            </span>
                                            <span class="text-xs text-gray-400"
                                                x-text="new Date(notification.created_at).toLocaleDateString() + ' ' + new Date(notification.created_at).toLocaleTimeString()">
                                            </span>
                                        </span>
                                    </a>
                                </li>
                            ...
                            <li x-show="notifications.length === 0" class="p-2 text-center text-gray-500">
                                No new notifications
                            </li>
                        </ul>

                        <a href="/notifications/all"
                            class="mt-4 flex justify-center rounded border border-gray-300 bg-white py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
            ...
            <!-- User Area -->
            <div class="relative" x-data="{ dropdownOpen: false, user: { name: '{{ Auth::guard('web')->user()->name }}', email: '{{ Auth::guard('web')->user()->email }}' } }" @click.outside="dropdownOpen = false"
                @profile-updated="user.name = $event.detail.name; user.email = $event.detail.email">
                
                <a class="flex items-start text-gray-700 dark:text-gray-400" href="#"
                    @click.prevent="dropdownOpen = !dropdownOpen">
                    <span class="mr-3 h-11 w-11 overflow-hidden rounded-full">
                        <img src="/images/users/user-profile.png" alt="User" />
                    </span>
                    <div class="flex flex-col">
                        <span class="mr-1 block text-theme-sm font-medium" x-text="user.name"></span>
                        <span class="block text-xs font-medium text-gray-500 dark:text-gray-500">Admin</span>
                    </div>
                ...
                <div x-show="dropdownOpen"
                    ...
                    <ul class="flex flex-col gap-1 border-b border-gray-200 pb-3 pt-4 dark:border-gray-800">
                        <li x-data="{
                            ...
                            submitForm() {
                                this.errors = {};
                                axios.post('{{ route('admin.profile') }}', this.form)
                                    .then(response => {
                            ...
                            {{-- Start Edit Form Popup  --}}
                            <div x-show="editing"
                                class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50"
                                @click.self="editing = false">
                                <div class="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-dark"
                                    @click.stop>
                                    <h3 class="mb-4 text-lg font-semibold text-gray-800 dark:text-white">Edit Profile
                                    </h3>
                                    <div class="space-y-4">
                                        <!-- Name Field -->
                                        <div>
                                            <input x-model="form.name" type="text" placeholder="Name"
                                                class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                                            <span x-show="errors.name" class="text-xs text-red-500"
                                                x-text="errors.name[0]"></span>
                                        </div>
                                        ...
                                        <div>
                                            <input x-model="form.email" type="email" placeholder="Email"
                                                class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                                            <span x-show="errors.email" class="text-xs text-red-500"
                                                x-text="errors.email[0]"></span>
                                        </div>

                                        <!-- Password Field -->
                                        <div>
                                            <input x-model="form.password" type="password" placeholder="New Password"
                                                class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                                            <span x-show="errors.password" class="text-xs text-red-500"
                                                x-text="errors.password[0]"></span>
                                        </div>

                                        <!-- Password Confirmation Field -->
                                        <div>
                                            <input x-model="form.password_confirmation" type="password"
                                        ...
                                        <div class="flex gap-2">
                                            <button @click="submitForm()"
                                                class="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm bg-black text-white dark:border-gray-700 dark:hover:bg-gray-700">
                                                Save
                                            </button>
                                            <button @click="editing = false"
                                                class="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-700">
                                                Cancel
                                            </button>
                                        </div>
                                    </div>
