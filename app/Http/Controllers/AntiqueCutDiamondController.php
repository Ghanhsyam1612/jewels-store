...
class AntiqueCutDiamondController extends Controller
{
    public function index(Request $request)
    {
        $query = AntiqueCutDiamond::query();
        ...
        if ($request->filled('depth_min') && $request->filled('depth_max')) {
            $query->whereBetween('depth', [(float)$request->depth_min, (float)$request->depth_max]);
        }
        ...
        if ($request->ajax()) {
            ...
            $view = view('components.antique_cut_diamond_rows', compact('diamonds'))->render();
        ...
        $diamonds = $query->inRandomOrder()->paginate(10);
        return view('components.antique-cut-diamonds', compact('diamonds'));
    }

    public function details($id)
    {
        $diamond = AntiqueCutDiamond::findOrFail($id);
        return view('components.antique-cut-diamonds', compact('diamond'));
    }
}
